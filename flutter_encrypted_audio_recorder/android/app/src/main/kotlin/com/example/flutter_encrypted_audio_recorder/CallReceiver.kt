...
class CallReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "flutter_call_receiver_prefs"
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (!numberFromIntent.isNullOrEmpty()) {
            ...
            Log.d(TAG, "Saved number to prefs: $numberFromIntent")
        }

        val stateStr = intent.getStringExtra(TelephonyManager.EXTRA_STATE) ?: return
        Log.d(TAG, "Phone State Change: $stateStr | Action: $action")
        ...
        when (stateStr) {
            TelephonyManager.EXTRA_STATE_RINGING -> {
                ...
                Log.d(TAG, "Incoming call ringing from: ${savedNumber ?: "Unknown"}")
            }
            TelephonyManager.EXTRA_STATE_OFFHOOK -> {
                ...
                Log.d(TAG, "Call OFFHOOK with number: $targetNumber")

                if (!isRecordingCall && !targetNumber.isNullOrEmpty()) {
                    if (dbHelper.isPhoneNumberInWhitelist(targetNumber)) {
                        Log.d(TAG, "Whitelist match for $targetNumber! Starting native background recording service.")
                        ...
                        Log.d(TAG, "Call active from $targetNumber, but number NOT in whitelist DB.")
                    }
                }
            }
