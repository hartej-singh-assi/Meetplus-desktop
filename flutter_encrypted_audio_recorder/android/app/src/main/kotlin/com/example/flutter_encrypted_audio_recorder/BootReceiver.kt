    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (action == Intent.ACTION_BOOT_COMPLETED ||
            ...
            if (isBackgroundEnabled) {
                Log.d(TAG, "Background call listener enabled in DB settings following system boot/update.")
            } else {
                Log.d(TAG, "Background call listener disabled in DB settings on boot.")
            }
        }
    }
}
