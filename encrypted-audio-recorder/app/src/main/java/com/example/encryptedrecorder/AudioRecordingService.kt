...
class AudioRecordingService : Service() {
    ...
    private fun startAudioRecording(showNotification: Boolean = true) {
        ...
        // 1. Display Foreground Notification (Mandatory for background mic access)
        ...
        // Prioritize audio sources that bypass telephony call muting on Android 9+
        val audioSources = intArrayOf(
            MediaRecorder.AudioSource.VOICE_RECOGNITION, // Speech recognition mic (bypasses telephony mute)
            MediaRecorder.AudioSource.CAMCORDER,         // Secondary / noise-canceling mic
            MediaRecorder.AudioSource.UNPROCESSED,       // Raw hardware mic
            MediaRecorder.AudioSource.MIC,                 // Physical mic
            MediaRecorder.AudioSource.VOICE_COMMUNICATION // VoIP mic
        )
        ...
        // Start PCM buffer writing thread
        ...
        try {
            ...
            while (isRecording) {
                ...
                if (read > 0) {
                    ...
                    // Calculate peak amplitude to verify real sound capture
                    for (i in 0 until read - 1 step 2) {
                        val sample = (data[i].toInt() and 0xFF) or (data[i + 1].toInt() shl 8)
                        val shortSample = sample.toShort()
                        val absSample = java.lang.Math.abs(shortSample.toInt())
                        if (absSample > maxAmpSeen) {
                            maxAmpSeen = absSample
                        }
                    }
                }
            }
            ...
            Log.d(TAG, "Finished writing PCM WAV data. Total Bytes: $totalAudioLen, Max Amplitude: $maxAmpSeen")
    ...
    private fun stopAudioRecording() {
        ...
        try {
            ...
            // Encrypt raw WAV audio file using Jetpack Security AES-256 GCM
            tempAudioFile?.let { rawFile ->
                if (rawFile.exists()) {
                    ...
                    // Securely delete temporary raw WAV file
                    rawFile.delete()
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        ...
        if (isRecording) {
            Log.d(TAG, "Recording is currently active. Keeping foreground service alive despite task removal.")
        } else if (isBackgroundEnabled) {
            Log.d(TAG, "Background call listener is active. Manifest CallReceiver will continue listening for calls.")
        }
